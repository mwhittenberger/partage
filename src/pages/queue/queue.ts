...
export class QueuePage {
    ...
    relationshipMessageIndex: number;
    relationshipStatusIndex: number;
    ...
    constructor(public toast: ToastController, public navCtrl: NavController, public navParams: NavParams, public wpService: WpServiceProvider, public storage: Storage, private modal: ModalController) {
        this.storage.get('fbToken').then((val) => {
            ...
            this.wpUser.subscribe(data => {
                ...
                this.connections = this.wpService.getQueue(this.userCookie);
                this.connections.subscribe(
                    data => {
                        ...
                        this.relationshipMessageIndex = this.count + 2;
                        this.relationshipStatusIndex = this.count + 3;
                        this.thisPartnership = data[this.count];
                        this.partnershipId = data[this.partnershipIdIndex];
                        this.thisMessage = data[this.relationshipMessageIndex];
                        this.thisStatus = data[this.relationshipStatusIndex];
    ...
    updateView() {
        ...
        if(this.thisStatus == 'approved') {
            this.showimages = 'showimages';
        }
        else {
            this.showimages = 'hideimages';
        }
        if(this.thisMessage.includes("Waiting")) {
            this.showui = 'hideui';
        }
        else {
            this.showui = 'showui';
    ...
    next() {
        ...
        if(this.skipme.indexOf(this.partnershipIdIndex) > -1) {
            ...
            this.thisMessage = this.possibleConnections[this.relationshipMessageIndex];
            this.thisStatus = this.possibleConnections[this.relationshipStatusIndex];
            this.updateView();
        }
    ...
    thumbsup() {
        ...
        const toast = this.toast.create({
            message: 'We will alert your partner that you have expessed interest in this couple.',
            duration: 5000
        });
        toast.present();
